...
init(Config) ->
    ...
    ?LOG_INFO(#{msg => "Initialize timeseries_server",
                backend => Backend,
...
handle_call({new, Token}, _, #state{backend = Backend1} = State) ->
    ...
    case backend_apply(Backend1, is_available, [Token]) of
        {true, Backend2} ->
            Timeseries = timeseries:new(Token),
            {ok, Backend3} = backend_apply(Backend2, save, [Timeseries]),
            {reply, ok, State#state{backend = Backend3}};
        {false, Backend2} ->
            {reply, {error, token_already_exist},
             State#state{backend = Backend2}}
    end;
...
handle_call({finish, Token}, _, #state{backend = Backend1} = State) ->
    ?LOG_NOTICE(#{msg => "Finish",
                  token => Token}),

    {Result, Backend2} = backend_apply(Backend1, finish, [Token]),
    {reply, Result, State#state{backend = Backend2}};
