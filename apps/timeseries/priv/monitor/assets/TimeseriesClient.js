...
var $author$project$TimeseriesClient$initChartConfig = F4(
	function (mXDim, mYDim, name, timeseries) {
		var yDim = A2($elm$core$Maybe$withDefault, '', mYDim);
		var xDim = A2($elm$core$Maybe$withDefault, '', mXDim);
		var sampleData = $elm$core$List$head(timeseries);
		var dims = function () {
			if (sampleData.$ === 'Nothing') {
				return _List_Nil;
			} else {
				var sample = sampleData.a;
				return $elm$core$Dict$keys(sample);
			}
		}();
		var initXDim = (_Utils_eq(mXDim, $elm$core$Maybe$Nothing) || (!A2($elm$core$List$member, xDim, dims))) ? A2(
			$elm$core$Maybe$withDefault,
			't',
			$elm$core$List$head(dims)) : xDim;
		var mXDimValues = A2(
			$elm$core$List$map,
			$elm$core$Dict$get(initXDim),
			timeseries);
		var xDimValues = A2(
			$elm$core$List$map,
			$elm$core$Maybe$withDefault(0),
			mXDimValues);
		var initYDim = (_Utils_eq(mYDim, $elm$core$Maybe$Nothing) || (!A2($elm$core$List$member, yDim, dims))) ? A2(
			$elm$core$Maybe$withDefault,
			initXDim,
			$elm$core$List$head(
				A2($elm$core$List$drop, 1, dims))) : yDim;
		var mYDimValues = A2(
			$elm$core$List$map,
			$elm$core$Dict$get(initYDim),
			timeseries);
		var yDimValues = A2(
			$elm$core$List$map,
			$elm$core$Maybe$withDefault(0),
		...
		return {
			...
			xMax: A2(
				$elm$core$Maybe$withDefault,
				1,
				$elm$core$List$maximum(xDimValues)),
			xMin: A2(
				$elm$core$Maybe$withDefault,
				0,
				$elm$core$List$minimum(xDimValues)),
			yDim: initYDim,
			yMax: A2(
				$elm$core$Maybe$withDefault,
				1,
				$elm$core$List$maximum(yDimValues)),
			yMin: A2(
				$elm$core$Maybe$withDefault,
				0,
...
var $author$project$TimeseriesClient$CookieLoaded = function (a) {
	return {$: 'CookieLoaded', a: a};
};
...
var $author$project$TimeseriesClient$NewTimeseriesName = F2(
	function (a, b) {
		return {$: 'NewTimeseriesName', a: a, b: b};
	});
...
var $author$project$TimeseriesClient$GotTimeseries = F2(
	function (a, b) {
		return {$: 'GotTimeseries', a: a, b: b};
	});
...
var $author$project$TimeseriesClient$downloadTimeseries = function (name) {
	return $elm$http$Http$get(
		{
			expect: $elm$http$Http$expectString(
				$author$project$TimeseriesClient$GotTimeseries(name)),
			url: '/download/' + name
		});
};
var $author$project$TimeseriesClient$emptyChartConfig = A4($author$project$TimeseriesClient$initChartConfig, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing, '', _List_Nil);
...
var $author$project$TimeseriesClient$dataToPoint = F3(
	function (xDim, yDim, dict) {
		...
		var justY = A2($elm$core$Maybe$withDefault, 0, y);
		var justX = A2($elm$core$Maybe$withDefault, 0, x);
		return A2($author$project$TimeseriesClient$Point, justX, justY);
	});
...
var $author$project$TimeseriesClient$derivate = function (data) {
	var yy = A2(
		...
		data);
	var yDiffs = $author$project$TimeseriesClient$differences(yy);
	var xx = A2(
		$elm$core$List$map,
		function ($) {
			return $.x;
...
var $author$project$TimeseriesClient$points = F2(
	function (timeseries, config) {
		...
		var originalPoints = A2($elm$core$List$map, currentDataToPoint, timeseries);
		return config.derivated ? $author$project$TimeseriesClient$derivate(originalPoints) : originalPoints;
	});
var $elm$core$Basics$round = _Basics_round;
var $author$project$TimeseriesClient$toMaybe = function (x) {
	return $elm$core$List$head(
		_List_fromArray(
			[x]));
};
var $author$project$TimeseriesClient$updateConfig = F2(
	function (model, _v0) {
		var chartIdx = _v0.a;
		var updateChartConfig = _v0.b;
		var change = F2(
			function (idx, chartConfig) {
				return _Utils_eq(idx, chartIdx) ? updateChartConfig(chartConfig) : chartConfig;
			});
		...
		while (true) {
			switch (msg.$) {
				case 'GotInfo':
					if (msg.a.$ === 'Ok') {
						...
						if (_v1.$ === 'Nothing') {
							...
							var timeseriesName = _v1.a;
							var mFirstConfig = $elm$core$List$head(model.config);
							var firstConfig = A2($elm$core$Maybe$withDefault, $author$project$TimeseriesClient$emptyChartConfig, mFirstConfig);
							if (firstConfig.timeseriesName === '') {
								var $temp$msg = A2($author$project$TimeseriesClient$NewTimeseriesName, 0, timeseriesName),
				...
				case 'GotTimeseries':
					if (msg.b.$ === 'Ok') {
						var name = msg.a;
						var timeseriesString = msg.b.a;
						var decoder = $elm$json$Json$Decode$list(
							$elm$json$Json$Decode$dict($elm$json$Json$Decode$float));
						var decoded = A2($elm$json$Json$Decode$decodeString, decoder, timeseriesString);
						...
						var change = function (config) {
							return (_Utils_eq(config.timeseriesName, name) || (config.timeseriesName === '')) ? A4($author$project$TimeseriesClient$initChartConfig, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing, name, timeseries) : config;
						};
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									...
									timeseries: A3($elm$core$Dict$insert, name, timeseries, model.timeseries)
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						var name = msg.a;
				...
				case 'Tick':
					var change = function (chartConfig) {
						if (chartConfig.paused) {
							return chartConfig;
						} else {
							var mLength = A2($elm$core$Dict$get, chartConfig.timeseriesName, model.timeseriesInfo);
							var length = A2($elm$core$Maybe$withDefault, 0, mLength);
							var currentLength = chartConfig.derivated ? (length - 1) : length;
							var lastIdx = function () {
								var _v3 = chartConfig.animationType;
								switch (_v3.$) {
									case 'None':
										return 0;
									case 'PointByPoint':
									...
									default:
										return currentLength - $author$project$TimeseriesClient$n;
								}
							}();
							return (_Utils_cmp(chartConfig.animationIdx, lastIdx) < 0) ? _Utils_update(
								chartConfig,
								{animationIdx: chartConfig.animationIdx + 1}) : _Utils_update(
								chartConfig,
								{paused: true});
						}
					};
				...
				case 'CookieLoaded':
					var value = msg.a;
					var splitted = A2($elm$core$String$split, '=', value);
					var mTimeseriesName = $elm$core$List$head(
						$elm$core$List$reverse(splitted));
					var timeseriesName = A2($elm$core$Maybe$withDefault, '', mTimeseriesName);
					var $temp$msg = A2($author$project$TimeseriesClient$NewTimeseriesName, 0, timeseriesName),
						$temp$model = model;
					msg = $temp$msg;
					model = $temp$model;
					continue update;
				case 'NewTimeseriesName':
					var chartIdx = msg.a;
					var newTimeseriesName = msg.b;
					var updateChartConfig = function (chartConfig) {
						...
						var sampleData = $elm$core$List$head(newTimeseries);
						var newDims = function () {
							if (sampleData.$ === 'Nothing') {
								return _List_Nil;
							} else {
								var sample = sampleData.a;
						...
						var xDimInNewDims = A2($elm$core$List$member, chartConfig.xDim, newDims);
						var yDimInNewDims = A2($elm$core$List$member, chartConfig.yDim, newDims);
						return (xDimInNewDims && yDimInNewDims) ? A4(
							$author$project$TimeseriesClient$initChartConfig,
							$author$project$TimeseriesClient$toMaybe(chartConfig.xDim),
							$author$project$TimeseriesClient$toMaybe(chartConfig.yDim),
							newTimeseriesName,
							newTimeseries) : A4($author$project$TimeseriesClient$initChartConfig, $elm$core$Maybe$Nothing, $elm$core$Maybe$Nothing, newTimeseriesName, newTimeseries);
					};
					var cmd = A2($elm$core$Dict$member, newTimeseriesName, model.timeseries) ? $elm$core$Platform$Cmd$none : $author$project$TimeseriesClient$downloadTimeseries(newTimeseriesName);
					return _Utils_Tuple2(
						A2(
							$author$project$TimeseriesClient$updateConfig,
							model,
							_Utils_Tuple2(chartIdx, updateChartConfig)),
						cmd);
				case 'NewXAxis':
					var chartIdx = msg.a;
					var newXAxis = msg.b;
					var updateChartConfig = function (chartConfig) {
						var mTimeseries = A2($elm$core$Dict$get, chartConfig.timeseriesName, model.timeseries);
						var timeseries = A2($elm$core$Maybe$withDefault, _List_Nil, mTimeseries);
						var newPoints = A2(
						...
						return _Utils_update(
							chartConfig,
							{
								...
								xMax: A2(
									$elm$core$Maybe$withDefault,
									1,
									$elm$core$List$maximum(newX)),
								xMin: A2(
									$elm$core$Maybe$withDefault,
									0,
									$elm$core$List$minimum(newX))
							});
					};
					return _Utils_Tuple2(
						A2(
						...
						return _Utils_update(
							chartConfig,
							{
								...
								yMax: A2(
									$elm$core$Maybe$withDefault,
									1,
									$elm$core$List$maximum(newY)),
								yMin: A2(
									$elm$core$Maybe$withDefault,
									0,
									$elm$core$List$minimum(newY))
							});
					};
					return _Utils_Tuple2(
						A2(
				...
				case 'PlotDerivate':
					...
					var updateChartConfig = function (chartConfig) {
						if (chartConfig.derivated) {
							...
							return _Utils_update(
								chartConfig,
								{derivated: true, yMax: yMax, yMin: yMin});
						}
					};
					return _Utils_Tuple2(
						A2(
					...
					var updateChartConfig = function (chartConfig) {
						if (chartConfig.derivated) {
							...
							var yMax = A2(
								$elm$core$Maybe$withDefault,
								1,
								$elm$core$List$maximum(originalY));
							var yMin = A2(
								$elm$core$Maybe$withDefault,
								0,
								$elm$core$List$minimum(originalY));
							return _Utils_update(
								chartConfig,
								{derivated: false, yMax: yMax, yMin: yMin});
						} else {
							return chartConfig;
						}
					};
				...
				case 'ZoomIn':
					var chartIdx = msg.a;
					var updateChartConfig = function (chartConfig) {
						var yRange = chartConfig.yMax - chartConfig.yMin;
						var xRange = chartConfig.xMax - chartConfig.xMin;
						return _Utils_update(
							chartConfig,
							{xMax: chartConfig.xMax - (0.1 * xRange), xMin: chartConfig.xMin + (0.1 * xRange), yMax: chartConfig.yMax - (0.1 * yRange), yMin: chartConfig.yMin + (0.1 * yRange)});
					};
					return _Utils_Tuple2(
						A2(
							$author$project$TimeseriesClient$updateConfig,
				...
				case 'ZoomOut':
					var chartIdx = msg.a;
					var updateChartConfig = function (chartConfig) {
						var yRange = (chartConfig.yMax - chartConfig.yMin) / 0.8;
						var xRange = (chartConfig.xMax - chartConfig.xMin) / 0.8;
						return _Utils_update(
							chartConfig,
							{xMax: chartConfig.xMax + (0.1 * xRange), xMin: chartConfig.xMin - (0.1 * xRange), yMax: chartConfig.yMax + (0.1 * yRange), yMin: chartConfig.yMin - (0.1 * yRange)});
					};
				...
				case 'DragMode':
					...
					var updateChartConfig = function (chartConfig) {
						return _Utils_update(
							chartConfig,
							{mouseMode: $author$project$TimeseriesClient$Drag});
					};
					return _Utils_Tuple2(
						A2(
							$author$project$TimeseriesClient$updateConfig,
				...
				case 'ResetAxis':
					...
					var updateChartConfig = function (chartConfig) {
						var mTimeseries = A2($elm$core$Dict$get, chartConfig.timeseriesName, model.timeseries);
						var timeseries = A2($elm$core$Maybe$withDefault, _List_Nil, mTimeseries);
						var currentPoints = A2($author$project$TimeseriesClient$points, timeseries, chartConfig);
						var currentX = A2(
							$elm$core$List$map,
							function ($) {
								return $.x;
						...
						return _Utils_update(
							chartConfig,
							{
								xMax: A2(
									$elm$core$Maybe$withDefault,
									1,
									$elm$core$List$maximum(currentX)),
								xMin: A2(
									$elm$core$Maybe$withDefault,
									0,
									$elm$core$List$minimum(currentX)),
								yMax: A2(
									$elm$core$Maybe$withDefault,
									1,
				...
				case 'StartSlidingWindow':
					...
					var updateChartConfig = function (chartConfig) {
						return _Utils_update(
							chartConfig,
							{animationIdx: -1, animationType: $author$project$TimeseriesClient$SlidingWindow, paused: false});
					};
					return _Utils_Tuple2(
						A2(
							$author$project$TimeseriesClient$updateConfig,
				...
				case 'StartPointByPoint':
					...
					var updateChartConfig = function (chartConfig) {
						return _Utils_update(
							chartConfig,
							{animationIdx: -1, animationType: $author$project$TimeseriesClient$PointByPoint, paused: false});
					};
					return _Utils_Tuple2(
						A2(
							$author$project$TimeseriesClient$updateConfig,
				...
				case 'PauseContinue':
					...
					var updateChartConfig = function (chartConfig) {
						...
						var currentLength = chartConfig.derivated ? (length - 1) : length;
						var lastIdx = function () {
							var _v6 = chartConfig.animationType;
							switch (_v6.$) {
								case 'None':
									return 0;
								case 'PointByPoint':
						...
						return _Utils_eq(chartConfig.animationIdx, lastIdx) ? _Utils_update(
							chartConfig,
							{animationIdx: -1, paused: false}) : _Utils_update(
							chartConfig,
							{paused: !chartConfig.paused});
					};
					return _Utils_Tuple2(
						A2(
							$author$project$TimeseriesClient$updateConfig,
				...
				case 'Hold':
					...
					var updateChartConfig = function (chartConfig) {
						return _Utils_update(
							chartConfig,
							{
								last: $author$project$TimeseriesClient$toMaybe(point)
							});
					};
					return _Utils_Tuple2(
						A2(
				...
				case 'Move':
					...
					var point = msg.b;
					var updateChartConfig = function (chartConfig) {
						var _v7 = chartConfig.mouseMode;
						if (_v7.$ === 'Zoom') {
							return _Utils_update(
								chartConfig,
								{current: point, thereIsHint: true});
						} else {
							var _v8 = chartConfig.last;
							if (_v8.$ === 'Nothing') {
								return _Utils_update(
									chartConfig,
									{current: point, thereIsHint: true});
								...
								return _Utils_update(
									chartConfig,
									{thereIsHint: true, xMax: chartConfig.xMax + currentDistX, xMin: chartConfig.xMin + currentDistX, yMax: chartConfig.yMax + currentDistY, yMin: chartConfig.yMin + currentDistY});
							}
						}
					};
					return _Utils_Tuple2(
				...
				case 'Drop':
					...
					var point = msg.b;
					var updateChartConfig = function (chartConfig) {
						var _v9 = chartConfig.last;
						if (_v9.$ === 'Nothing') {
							return chartConfig;
						} else {
							var last = _v9.a;
							var _v10 = chartConfig.mouseMode;
							if (_v10.$ === 'Drag') {
								return _Utils_update(
									chartConfig,
									{last: $elm$core$Maybe$Nothing});
								...
								if ((_Utils_cmp(xDist, xRange * 0.05) > 0) && (_Utils_cmp(yDist, yRange * 0.05) > 0)) {
									var yTo = A2($elm$core$Basics$max, last.y, point.y);
									var yFrom = A2($elm$core$Basics$min, last.y, point.y);
									var xTo = A2($elm$core$Basics$max, last.x, point.x);
									var xFrom = A2($elm$core$Basics$min, last.x, point.x);
									return _Utils_update(
										chartConfig,
										{last: $elm$core$Maybe$Nothing, xMax: xTo, xMin: xFrom, yMax: yTo, yMin: yFrom});
								} else {
									return _Utils_update(
										chartConfig,
										{last: $elm$core$Maybe$Nothing});
...
var $author$project$TimeseriesClient$chartControlView = F4(
...
var $author$project$TimeseriesClient$listFromRange = F3(
	function (start, end, count) {
		...
		var roundedStart = $elm$core$Basics$round(start / scale) * scale;
		var roundedStep = $elm$core$Basics$round(step / scale) * scale;
...
var $author$project$TimeseriesClient$customTicks = F3(
	function (start, end, count) {
		...
		var listEnd = $elm$core$List$head(
			A2(
				$elm$core$Maybe$withDefault,
				_List_Nil,
				$elm$core$List$tail(list)));
		var listHead = $elm$core$List$head(list);
		var step = A2($elm$core$Maybe$withDefault, 1, listEnd) - A2($elm$core$Maybe$withDefault, 0, listHead);
		var decimals = $elm$core$Basics$floor(
...
var $author$project$TimeseriesClient$events = function (idx) {
	...
	var event = $terezka$line_charts$LineChart$Events$onWithOptions;
...
var $author$project$TimeseriesClient$hintOfPoint = F2(
	function (config, point) {
		var yRounded = A2($myrho$elm_round$Round$round, 2, point.y);
		var xRounded = A2($myrho$elm_round$Round$round, 2, point.x);
		return _Utils_Tuple2(config.xDim + (': ' + xRounded), config.yDim + (': ' + yRounded));
	});
...
var $author$project$TimeseriesClient$hintView = F4(
	function (_v0, currentPoints, config, system) {
		var height = _v0.a;
		var width = _v0.b;
		if (config.thereIsHint) {
			var xLabel = system.x.max + ((system.x.max - system.x.min) * 0.05);
			var labelHeight = ((system.y.max - system.y.min) / height) * 50;
			var distsq = F2(
				function (idx, point) {
					var yDist = point.y - config.current.y;
					var xDist = point.x - config.current.x;
					return {
						distsq: A2($elm$core$Basics$pow, xDist, 2) + A2($elm$core$Basics$pow, yDist, 2),
						idx: idx,
						point: point
					};
				});
			var distsqs = A2(
				...
				A2($elm$core$List$indexedMap, distsq, currentPoints));
			var _v1 = function () {
				var _v2 = $elm$core$List$head(distsqs);
				if (_v2.$ === 'Just') {
					var min = _v2.a;
					return (_Utils_cmp(
						min.distsq,
						A2($elm$core$Basics$pow, (config.xMax - config.xMin) * 0.03, 2)) < 0) ? _Utils_Tuple2(min.point, $terezka$line_charts$LineChart$Colors$strongBlue) : _Utils_Tuple2(config.current, $terezka$line_charts$LineChart$Colors$black);
				} else {
			...
			var xHint = _v3.a;
			var yHint = _v3.b;
			var yLabel = A2($elm$core$Basics$min, hinted.y, system.y.max - (2 * labelHeight));
			var yLabelDown = yLabel - labelHeight;
			return _List_fromArray(
				[
					A8($terezka$line_charts$LineChart$Junk$labelAt, system, xLabel, yLabel, system.x.min, system.y.min, 'right', hintColor, xHint),
					A8($terezka$line_charts$LineChart$Junk$labelAt, system, xLabel, yLabelDown, system.x.min, system.y.min, 'right', hintColor, yHint)
				]);
		} else {
			return _List_Nil;
		}
...
var $author$project$TimeseriesClient$chart = F5(
	function (timeseries, width, twoColumns, chartIdx, chartConfig) {
		var mCurrentTimeseries = A2($elm$core$Dict$get, chartConfig.timeseriesName, timeseries);
		var idx = A2($elm$core$Basics$max, 0, chartConfig.animationIdx);
		var height = $elm$core$Basics$round(width * 0.5);
		var _float = (twoColumns && (!(!A2($elm$core$Basics$modBy, 2, chartIdx)))) ? 'float_right' : 'float_left';
		var currentTimeseries = A2($elm$core$Maybe$withDefault, _List_Nil, mCurrentTimeseries);
		var currentPoints = A2($author$project$TimeseriesClient$points, currentTimeseries, chartConfig);
		var lastIdx = function () {
			var _v1 = chartConfig.animationType;
			switch (_v1.$) {
				case 'None':
					return 0;
				case 'PointByPoint':
		...
		var pauseButtonText = chartConfig.paused ? ((_Utils_cmp(chartConfig.animationIdx, lastIdx) < 0) ? 'Continue' : 'Start over') : 'Pause';
		var pointsToPlot = function () {
			var _v0 = chartConfig.animationType;
			switch (_v0.$) {
				case 'None':
		...
		var color = chartConfig.derivated ? $terezka$line_charts$LineChart$Colors$blue : $terezka$line_charts$LineChart$Colors$strongBlue;
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					A4($author$project$TimeseriesClient$chartControlView, chartIdx, chartConfig.mouseMode, pauseButtonText, chartConfig.animationType),
					A2(
					$terezka$line_charts$LineChart$viewCustom,
					{
						...
						junk: $terezka$line_charts$LineChart$Junk$custom(
							function (system) {
								return {
									above: A4(
										$author$project$TimeseriesClient$hintView,
									...
									below: _List_fromArray(
										[
											A2($author$project$TimeseriesClient$zoomRect, chartConfig, system)
										]),
									html: _List_Nil
								};
							}),
						...
						x: $terezka$line_charts$LineChart$Axis$custom(
							{
								axisLine: $terezka$line_charts$LineChart$Axis$Line$custom(
									F2(
										function (pointRange, axisRange) {
											return {color: $terezka$line_charts$LineChart$Colors$gray, end: chartConfig.xMax, events: _List_Nil, start: chartConfig.xMin, width: 2};
										})),
								pixels: width,
								range: A2($terezka$line_charts$LineChart$Axis$Range$window, chartConfig.xMin, chartConfig.xMax),
								ticks: A3($author$project$TimeseriesClient$customTicks, chartConfig.xMin, chartConfig.xMax, 8),
								title: $terezka$line_charts$LineChart$Axis$Title$default(chartConfig.xDim),
								variable: A2(
									$elm$core$Basics$composeL,
									$elm$core$Maybe$Just,
									function ($) {
						...
						y: $terezka$line_charts$LineChart$Axis$custom(
							{
								axisLine: $terezka$line_charts$LineChart$Axis$Line$custom(
									F2(
										function (pointRange, axisRange) {
											return {color: $terezka$line_charts$LineChart$Colors$gray, end: chartConfig.yMax, events: _List_Nil, start: chartConfig.yMin, width: 2};
										})),
								pixels: height,
								range: A2($terezka$line_charts$LineChart$Axis$Range$window, chartConfig.yMin, chartConfig.yMax),
								ticks: A3($author$project$TimeseriesClient$customTicks, chartConfig.yMin, chartConfig.yMax, 8),
								title: $terezka$line_charts$LineChart$Axis$Title$default(chartConfig.yDim),
								variable: A2(
									$elm$core$Basics$composeL,
									$elm$core$Maybe$Just,
									function ($) {
					...
					_List_fromArray(
						[
							A4($terezka$line_charts$LineChart$line, color, $terezka$line_charts$LineChart$Dots$circle, chartConfig.timeseriesName, pointsToPlot)
						]))
				]));
	});
...
var $author$project$TimeseriesClient$timeseriesOption = F2(
	function (timeseriesName, opt) {
		return A2(
			$elm$html$Html$option,
			_List_fromArray(
				[
...
var $author$project$TimeseriesClient$chartConfigView = F4(
	function (dimOptions, timeseriesOptions, idx, config) {
		var _v0 = config.derivated ? _Utils_Tuple2('active', 'inactive') : _Utils_Tuple2('inactive', 'active');
		var derivateClass = _v0.a;
		var originalClass = _v0.b;
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
				...
				[
						...
						[
							...
							_List_fromArray(
								[
									...
									A2(
										$elm$json$Json$Decode$map,
										$author$project$TimeseriesClient$NewTimeseriesName(idx),
										$elm$html$Html$Events$targetValue))
								]),
							A2(
								$elm$core$List$map,
								$author$project$TimeseriesClient$timeseriesOption(config.timeseriesName),
								timeseriesOptions))
						])),
					A2(
					$elm$html$Html$button,
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$class(originalClass)
						]),
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$class(derivateClass)
						]),
					_List_fromArray(
						[
...
var $author$project$TimeseriesClient$columnConfigView = function (columns) {
	var _v0 = function () {
		if (columns.$ === 'One') {
			return _Utils_Tuple2('active', 'inactive');
		} else {
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$class(oneClass)
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$class(twoClass)
					]),
				_List_fromArray(
					[
...
var $author$project$TimeseriesClient$configurationView = function (model) {
	var timeseriesOptions = $elm$core$Dict$keys(model.timeseriesInfo);
	var dimOptions = function (idx) {
		var mCurrentChartConfig = $elm$core$List$head(
			A2($elm$core$List$drop, idx, model.config));
		var currentChartConfig = A2($elm$core$Maybe$withDefault, $author$project$TimeseriesClient$emptyChartConfig, mCurrentChartConfig);
		var mTimeseries = A2($elm$core$Dict$get, currentChartConfig.timeseriesName, model.timeseries);
		var timeseries = A2($elm$core$Maybe$withDefault, _List_Nil, mTimeseries);
		var sampleData = $elm$core$List$head(timeseries);
		if (sampleData.$ === 'Nothing') {
			return _List_Nil;
	...
	var currentChartConfigView = function (idx) {
		return A3(
			$author$project$TimeseriesClient$chartConfigView,
			dimOptions(idx),
			timeseriesOptions,
			idx);
	};
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$TimeseriesClient$columnConfigView(model.columns),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
				...
				A2($elm$core$List$indexedMap, currentChartConfigView, model.config)),
				A2(
				$elm$html$Html$button,
				_List_fromArray(
					[
...
var $author$project$TimeseriesClient$timeseriesInfoView = F2(
	function (timeseriesNames, info) {
		var toTableRow = function (timeseriesName) {
			var mLength = A2($elm$core$Dict$get, timeseriesName, info);
			var length = A2($elm$core$Maybe$withDefault, 0, mLength);
			return A2(
				$elm$html$Html$tr,
				_List_Nil,
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(timeseriesName)
							])),
						A2(
						$elm$html$Html$td,
						_List_Nil,
			...
			$elm$core$List$concat(
				_List_fromArray(
					[
						...
						A2($elm$core$List$map, toTableRow, timeseriesNames)
					])));
	});
...
var $author$project$TimeseriesClient$view = function (model) {
	var isTimeseries = function (name) {
		return name !== '';
	};
	var currentTimeseriesNames = $elm_community$list_extra$List$Extra$unique(
		A2(
			$elm$core$List$map,
			function ($) {
			...
			model.config));
	var timeseriesNames = A2($elm$core$List$filter, isTimeseries, currentTimeseriesNames);
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$author$project$TimeseriesClient$configurationView(model),
						A2(
						$elm$html$Html$div,
						_List_fromArray(
							[
						...
						_List_fromArray(
							[
								...
								A2($author$project$TimeseriesClient$timeseriesInfoView, timeseriesNames, model.timeseriesInfo)
							]))
					])),
