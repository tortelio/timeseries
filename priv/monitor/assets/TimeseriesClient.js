...
var author$project$TimeseriesClient$init = function (_n0) {
		...
		while (true) {
			switch (msg.$) {
				case 'NewDatasetName':
					...
					var cmd = function () {
						var _n3 = A2(elm$core$Dict$member, newDatasetName, model.datasets);
						if (_n3) {
					...
					var change = F2(
						function (idx, config) {
							if (_Utils_eq(idx, chartIdx)) {
								...
								var _n1 = A2(elm$core$List$member, config.xDim, new_dims) && A2(elm$core$List$member, config.yDim, new_dims);
								if (!_n1) {
				...
				case 'GotInfo':
					if (msg.a.$ === 'Ok') {
						var info = msg.a.a;
						var _n5 = elm$core$Dict$isEmpty(model.timeseriesInfo);
						if (!_n5) {
							return _Utils_Tuple2(
								_Utils_update(
									model,
				...
				case 'Drop':
					...
					var change = F2(
						function (idx, config) {
							if (_Utils_eq(idx, chartIdx)) {
								var _n10 = config.last;
								if (_n10.$ === 'Nothing') {
									return config;
								} else {
									var last = _n10.a;
									var _n11 = config.mouseMode;
									if (_n11.$ === 'Drag') {
										return _Utils_update(
											config,
										...
										var _n12 = (_Utils_cmp(xDist, xRange * 5.0e-2) > 0) && (_Utils_cmp(yDist, yRange * 5.0e-2) > 0);
										if (!_n12) {
											return _Utils_update(
												config,
												{last: elm$core$Maybe$Nothing, mouseButton: author$project$TimeseriesClient$Up});
...
var author$project$TimeseriesClient$animationButtons = F3(
	function (animationType, pauseText, chartIdx) {
		...
		var pointByPointClass = _n0.a;
		var slidingWindowClass = _n0.b;
...
var author$project$TimeseriesClient$sectionBand = F4(
	function (_n0, currentPoints, config, system) {
		var height = _n0.a;
		var width = _n0.b;
		if (config.thereIsHint) {
			var label_x = system.x.max + ((system.x.max - system.x.min) * 5.0e-2);
			...
			var _n1 = function () {
				var _n2 = elm$core$List$head(dists);
				if (_n2.$ === 'Just') {
					var min = _n2.a;
					var _n3 = _Utils_cmp(
						min.dist,
						A2(elm$core$Basics$pow, (config.xMax - config.xMin) * 3.0e-2, 2)) < 0;
					if (_n3) {
			...
			var hinted = _n1.a;
			var hintColor = _n1.b;
			var _n4 = A2(author$project$TimeseriesClient$hintOfPoint, config, hinted);
			var hint_x = _n4.a;
			var hint_y = _n4.b;
			var label_y = A2(elm$core$Basics$min, hinted.y, system.y.max - (2 * labelHeight));
			var label_y_down = label_y - labelHeight;
			return _List_fromArray(
				[
					...
					terezka$line_charts$LineChart$viewCustom,
					{
						...
						junk: terezka$line_charts$LineChart$Junk$custom(
							function (system) {
								return {
									above: A4(
										author$project$TimeseriesClient$sectionBand,
...
var author$project$TimeseriesClient$chartConfig = F4(
	function (dimOptions, datasetOptions, idx, config) {
		...
		var derivate_class = _n0.a;
		var original_class = _n0.b;
		return A2(
			elm$html$Html$div,
			_List_fromArray(
				[
...
var author$project$TimeseriesClient$datasetInfo = function (info) {
		...
		elm$core$List$concat(
			_List_fromArray(
				[
					...
					A2(elm$core$List$map, toTableRow, info)
				])));
};
...
var author$project$TimeseriesClient$view = function (model) {
	var toDict = function (datasetName) {
		var mLength = A2(elm$core$Dict$get, datasetName, model.timeseriesInfo);
		var length = A2(elm$core$Maybe$withDefault, 0, mLength);
		return {length: length, name: datasetName};
	};
	var currentDatasets = elm_community$list_extra$List$Extra$unique(
		A2(
			elm$core$List$map,
			function ($) {
				return $.datasetName;
			...
			model.config));
	var timeseriesInfo = A2(elm$core$List$map, toDict, currentDatasets);
	return A2(
		elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
					...
					[
						...
						_List_fromArray(
							[
								...
								author$project$TimeseriesClient$datasetInfo(timeseriesInfo)
							]))
					])),
